...
if (! function_exists('site_nav')) {
    ...
    function site_nav(?array $doktor = null): array
    {
        ...
        if (\Illuminate\Support\Facades\Route::has('frontend.hakkimda')) {
            $nav[] = ['href' => route('frontend.hakkimda'), 'label' => 'Hakkımda', 'match' => 'frontend.hakkimda', 'external' => false];
        }
        ...
        if (\Illuminate\Support\Facades\Route::has('frontend.egitimler') && ! empty($doktor['egitimler'])) {
            $nav[] = ['href' => route('frontend.egitimler'), 'label' => 'Eğitimler', 'match' => 'frontend.egitim*', 'external' => false];
        }
