    ...
    public function testConnection(?string $ePosta = null, ?string $sifre = null): array
    {
        ...
        try {
            ...
            if (empty($profile) || (empty($profile['id']) && empty($profile['ad_soyad']))) {
                return ['ok' => false, 'message' => 'API yanıt verdi ancak hekim profili boş.'];
            }

            $msg = 'Bağlantı başarılı · '.trim(($profile['unvan'] ?? '').' '.($profile['ad_soyad'] ?? 'Hekim'));
