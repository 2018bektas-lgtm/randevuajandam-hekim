...
class RequireApiIntegration
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Token yok: platform oturumu için yeniden giriş gerekir
        if (! $api->token()) {
            return redirect()
                ->route('panel.api-entegrasyon')
                ->with('uyari', 'API anahtarları kayıtlı ancak platform oturumu yok. Ana sunucu e-posta/şifrenizle yeniden giriş yapın veya bağlantıyı test edin.');
        }
