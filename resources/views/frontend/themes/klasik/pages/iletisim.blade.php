                            ...
                            <label class="meeting-type-option">
                                ...
                                <span class="meeting-type-icon" aria-hidden="true">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 21h18M5 21V9l7-5 7 5v12M9 21v-6h6v6M8 10h.01M16 10h.01"/>
                                    </svg>
                                </span>
...
(function () {
    ...
    async function loadServices() {
        ...
        try {
            ...
            const list = res.data || [];
            if (!list.length) {
                hizmetEl.innerHTML = '<option value="">Aktif hizmet bulunamadı</option>';
                return;
            }
            hizmetEl.innerHTML = '<option value="">Hizmet seçin</option>' +
                list.map(h => {
                    ...
                    return `<option value="${h.id}">${ad}${sure}</option>`;
                }).join('');
        } catch (e) {
            hizmetEl.innerHTML = '<option value="">Hizmetler şu an yüklenemedi</option>';
            showAlert(e.message || 'Randevu sistemi geçici olarak kullanılamıyor.');
        }
    }
    ...
    tarihEl?.addEventListener('change', () => {
        hideAlert();
        loadSlots();
    });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const d = res.data || {};
            const yonet = d.yonetim_url ? `<br><a href="${d.yonetim_url}" style="color:var(--brand-700);font-weight:700">Randevuyu yönet / iptal et →</a>` : '';
            const hesap = d.hesap_url ? `<br><a href="${d.hesap_url}" style="color:var(--brand-700);font-weight:700">Hesap oluştur →</a>` : '';
            const join = d.platform_join_url
                ? `<br><a href="${d.platform_join_url}" style="color:var(--brand-700);font-weight:700">Görüşmeye katıl (platform) →</a>`
