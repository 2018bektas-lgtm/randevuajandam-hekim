...
@endif
            ...
            <div class="section-title text-center" style="margin-bottom:2rem">
                @if(!empty($ayar['kucuk_baslik']))
                    <h3 class="wow fadeInUp">{{ $ayar['kucuk_baslik'] }}</h3>
                @endif
                @if(!empty($ayar['ana_baslik']))
                    <h2 class="text-anime-style-2" data-cursor="-opaque">{{ $ayar['ana_baslik'] }}</h2>
                @endif
...
<script>
(function () {
    if (window.__raWizardInit) return;
    window.__raWizardInit = true;
    if (!document.getElementById('ra-hizmetler')) return;
    const routes = {
